...
public class RClass {
	...
	/**
	 * 获取类的对象属性(序列化,基本数据类型,List除外)
	 * 
	 * @param clas
	 * @return
	 */
	public List<Field> getObjectOfFields() {
		String str = "Integer Long Short Byte Float Double Character Boolean Date String List";
		List<Field> list = new ArrayList<>();
		Field[] fields = clas.getDeclaredFields();
		for (Field f : fields) {
			logger.info(f.getType().getSimpleName());
			boolean bool = !str .contains( f.getType().toString());
			if( !f.getName().equals("serialVersionUID") && bool){
				list.add(f);
			}
		}
		return list;
